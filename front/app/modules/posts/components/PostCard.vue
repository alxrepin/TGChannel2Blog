...
<template>
  <UiCard variant="elevated" padding="none" class="flex height:240px">
    <div 
      :class="[
        'jc:space-between flex flex:column',
        hasImage ? 'width:70% p:20' : 'p:20 w:full'
      ]"
    >
      <div>
        ...
        <p class="f:14 pt:8 color:fade-54 lh:1.6 max-height:112px overflow:hidden lines:5 text:ellipsis box:content">
          {{ excerpt }}
        </p>
      </div>
      <time 
        :datetime="publishedAt" 
        class="f:14 color:fade-64"
      >
        {{ formattedDate }}
      </time>
    </div>
    ...
    <div 
      v-if="hasImage && thumbnail" 
      class="my:4 mr:4 r:24 width:30% overflow:hidden"
    >
      <NuxtImg
        :src="thumbnail"
        :alt="title"
